...
const DoctorDetailModal = ({ doctor, onClose }) => {
  // Mock data for expanded view - replace with actual data when available
  const doctorDetails = {
    education: "MD from AIIMS, Fellowship in Advanced Cardiology",
    experience: "15 years",
    languages: ["English", "Hindi", "Spanish"],
    procedures: ["General Consultation", "Cardiac Evaluation", "ECG Analysis"],
    availability: ["Mon-Wed: 9AM-5PM", "Fri: 10AM-3PM"],
    awards: ["Best Physician 2023", "Research Excellence Award"],
        ...
        <div className="px-4 pb-4">          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            <div>
              ...
              <p className="text-gray-600 text-xs mt-1">{doctorDetails.education}</p>
              <p className="text-gray-600 text-xs">{doctorDetails.experience} of experience</p>
              ...
              <p className="text-gray-600 text-xs mt-1">{doctorDetails.address}</p>
            </div>
            
            <div>
              <h3 className="font-semibold text-gray-800 text-sm">Procedures</h3>
              <div className="flex flex-wrap gap-1 mt-1">
                {doctorDetails.procedures.map((proc, index) => (
          ...
          <div className="flex gap-2 mt-4">
            <button className="flex-1 bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg transition-colors flex items-center justify-center gap-1 text-sm">
